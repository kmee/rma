...
class picking_out_from_exchange_lines(osv.osv_memory):
    ...
    _description = ('Wizard to create a picking out from selected exchange'
                    ' lines')
    _columns = {
    ...
    def _get_selected_lines(self, cr, uid, context):
        exchange_line_ids = self.pool.get('crm.claim').read(
            cr, uid, context['active_id'],
            ['product_exchange_ids'])['product_exchange_ids']
        exchange_lines = self.pool.get('product.exchange').browse(
            cr, uid, exchange_line_ids)
        m2m = []
            ...
            if True:  # line.selected:
                m2m.append(self.pool.get('temp.exchange.line').create(
                    cr, uid, {
                        ...
                        line.replacement_product_qty,
                        'replacement_prodlot_id':
                        line.replacement_product_serial.id,
                    }))
        return m2m
    ...
    def action_create_picking(self, cr, uid, ids, context=None):
        for exchange_lines in self.browse(cr, uid, ids):
            ...
            picking_id = self.pool.get('stock.picking').create(cr, uid, {
                ...
                'location_id': self.pool.get('stock.warehouse').read(
                    cr, uid, [1], ['lot_input_id'])[0]['lot_input_id'][0],
                'location_dest_id':
            ...
            for exchange_line in exchange_lines.exchange_line_ids:
                self.pool.get('stock.move').create(cr, uid, {
                    'name':
                    exchange_line.replacement_product_id.name_template,
