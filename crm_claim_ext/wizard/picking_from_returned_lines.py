...
class picking_in_from_returned_lines(osv.osv_memory):
    ...
    _columns = {
        'claim_line_location': fields.many2one(
            'stock.location', 'Dest. Location',
            help="Location where the system will stock the returned products.",
            select=True),
        'claim_line_ids': fields.many2many(
            'temp.claim.line', string='Selected return lines'),
    }
    ...
    def _get_selected_lines(self, cr, uid, context):
        ...
        returned_lines = self.pool.get('claim.line').browse(
    ...
    def _get_dest_loc(self, cr, uid, context):
        return self.pool.get('stock.warehouse').read(
        ...
        for picking in self.browse(cr, uid, ids):
            ...
            for picking_line in picking.claim_line_ids:
                self.pool.get('stock.move').create(cr, uid, {
                    'name': picking_line.product_id.name_template,
                    ...
                    'location_id': location,
                    'location_dest_id': picking.claim_line_location.id,
                    # self.pool.get('stock.warehouse').read(
                    #     cr, uid, [1]
