...
class exchange_from_returned_lines(osv.osv_memory):
    ...
    _columns = {
        'exchange_line_ids': fields.many2many(
            'temp.exchange.line', string='Selected exchange lines'),
    }

    # Get selected lines to add to exchange
    def _get_selected_lines(self, cr, uid, context):
        returned_line_ids = self.pool.get('crm.claim').read(
            cr, uid, context['active_id'],
            ['return_line_ids'])['return_line_ids']
        returned_lines = self.pool.get('return.line').browse(
            cr, uid, returned_line_ids)
        m2m = []
    ...
    def action_create_exchange(self, cr, uid, ids, context=None):
        for exchange in self.browse(cr, uid, ids):
            ...
            for line in exchange.exchange_line_ids:
                self.pool.get('product.exchange').create(cr, uid, {
                    'name': "#",
                    'state': 'draft',
...
class temp_exchange_line(osv.osv_memory):
    ...
    _columns = {
        'name': fields.char(
            'Comment', size=128, help="To describe the line product problem"),
        'returned_product_id': fields.many2one(
            'product.product', 'Returned product', required=True),
        'returned_product_quantity': fields.float(
            'Returned quantity', digits=(12, 2),
            help="Quantity of product returned"),
        'returned_unit_sale_price': fields.float(
            'Unit sale price', digits=(12, 2),),
        'returned_prodlot_id': fields.many2one(
            'stock.production.lot', 'Returned serial/Lot'),
        'replacement_product_id': fields.many2one(
            'product.product', 'Replacement product', required=True),
        'replacement_product_quantity': fields.float(
            'Replacement quantity', digits=(12, 2),
            help="Replacement quantity"),
        'replacement_prodlot_id': fields.many2one(
            'stock.production.lot', 'Replacement serial/Lot'),
    }
