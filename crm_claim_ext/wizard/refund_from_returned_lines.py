...
class refund_from_returned_lines(osv.osv_memory):
    ...
    _description = ('Wizard to create an refund for selected product return'
                    ' lines')
    _columns = {
        'refund_journal': fields.many2one(
            'account.journal', 'Refund journal', select=True),
    ...
    def _get_journal(self, cr, uid, context):
        ...
        return self.pool.get('account.journal').search(
            cr, uid, [('type', '=', 'sale_refund')], limit=1)[0]
            ...
            for refund_line in refund.claim_line_ids:
                if refund_line.invoice_id:
                    ...
                    raise osv.except_osv(
                        _('Error !'),
                        _('Cannot find any invoice for the return line!'))
        return {
            'name': 'Customer Refounds',
            'view_type': 'form',
            'view_mode': 'tree,form',
            'domain': (
                "[('type', '=', '" + invoice_type + "'),('partner_id','=',"
                + str(partner_id)+")]"),
