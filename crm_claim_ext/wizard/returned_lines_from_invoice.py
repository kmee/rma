...
class returned_lines_from_invoice_invoice(osv.osv_memory):
    ...
    def _get_default_partner_id(self, cr, uid, context):
        return self.pool.get('crm.claim').read(
            cr, uid, context['active_id'], ['partner_id'])['partner_id']
    ...
    def action_return_all(self, cr, uid, ids, context):
        ...
        invoice_lines_ids = invoice_line_pool.search(
            cr, uid, [('invoice_id', '=', inv_id)])
        # Get invoice lines from invoice line ids
        for invoice_line in invoice_line_pool.browse(
                cr, uid, invoice_lines_ids):
...
class returned_lines_from_invoice_lines(osv.osv_memory):
    ...
    _description = 'Wizard to create product return lines from invoice'
    _columns = {
        'claim_line_ids': fields.many2many('temp.claim.line',
                                           string='claim lines'),
    }
    ...
    def _get_possible_returns_from_invoice(self, cr, uid, context):
        ...
        invoice_lines_ids = self.pool.get('account.invoice.line').search(
            cr, uid, [('invoice_id', '=', context['invoice_id'])])
        m2m = []
